...
from model.utils import explode
...
class WindowMaker:
    ...
    # 한 boundary에 대해 windows, building 생성
    def create(self):
        # 모든 segments에 대해 windows 생성
        segments = explode(self.boundary)
        windows = []
        # 길이가 작은 segment 제외
        segments_selected = [segment for segment in segments if segment.Line.Length > 5]
    ...
    def create_window_on_segment(self, segment):
        ...
        def __divide_by_count(crv:rh.LineCurve,div_num:int):
        ...
        def create_window_from_baseline(_windowcurve):
            ...
            window = rh.Extrusion.Create(_windowcurve, 0.9, False)  # 높이 0.9m
            # window를 바닥면으로부터 높이기
        ...
        for windowcurve in window_baselines:
            translated_windows.append(create_window_from_baseline(windowcurve))
