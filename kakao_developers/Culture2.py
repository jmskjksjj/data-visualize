...
def visualize_building(building_info, nearby_buildings):
    ...
    for nearby_building in nearby_buildings:
        ...
        if nearby_building['category_group_code'] == 'AT4':
            folium.Marker([nearby_lat, nearby_lng], popup=nearby_name, icon=folium.Icon(color='red', icon_size=(100, 100))).add_to(m)
        else:
            folium.Marker([nearby_lat, nearby_lng], popup=nearby_name, icon=folium.Icon(color='green')).add_to(m)

    m.save("building_location_with_nearby.html")
    print("지도를 생성했습니다. 'building_location_with_nearby.html' 파일을 열어 확인하세요.")
